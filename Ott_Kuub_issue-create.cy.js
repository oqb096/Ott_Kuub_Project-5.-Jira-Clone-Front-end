describe('Ott Kuub', () => {
  ...
  it('Should validate title is required field if missing', () => {
   ...
   //Assert that the modal window is closed and a successful message is visible
   cy.get('[data-testid="modal:issue-create"]').should('not.exist');
   cy.contains('Issue has been successfully created.').should('be.visible');

   //The assertion of the disappeared message after the reload of the page
   cy.reload();
   cy.contains('Issue has been successfully created.').should('not.exist');

   //Assert that only one list with the name Backlog is visible and do steps inside of it
   cy.get('[data-testid="board-list:backlog"]').should('be.visible').and('have.length', '1').within(() => {
   ...
   cy.get('[data-testid="list-issue"]')
    ...
    //Assert that the correct type icon is visible
...
it('Test Case 2: Random Data Plugin Issue Creation', () => {
  ...
  cy.get('[data-testid="modal:issue-create"]').should('be.visible').within(() => {    
   ...
   //Select reporter "Pickle Rick"
   cy.get('[data-testid="select:userId"]').click();
   cy.get('[data-testid="select-option:Pickle Rick"]').trigger('click');
